...
Scenario: ERE replies with an error response to a malformed resolution request
  ...
  Contract reference: src/resources/schemas/er-schema.json (EREErrorResponse)
...
Scenario: An entity mention with an unsupported entity type is rejected by ERE
  ...
  Contract reference: src/resources/schemas/ere-service-schema.yaml (supported entity types)
